package com.smartstream.feeder;

import com.smartstream.common.Data;
import com.smartstream.common.SubData;
...
public class Feeder implements InitializingBean, DisposableBean {
	...
	public class FeederTask implements Runnable {
		...
		public void run() {
			try {
				if (atomicLong.getAndIncrement() > 5000) {
					destroy();
				}
				long time = System.currentTimeMillis();
				List<SubData> subDataList = new ArrayList<SubData>();
				for (int i = 0; i < 5; i++) {
					subDataList.add(new SubData("subDataString ",
							(i * atomicLong.intValue()) + (int) time));
				}
				Data data = new Data((counter++ % numberOfTypes), "FEEDER "
						+ Long.toString(time), subDataList);
